...
def train_reinforce(
    ...
    for episode in range(1, num_episodes+1):
        ...
        observations = rollout["observations"]  # exclude final obs
        ...
        ep_rewards = float(rollout["total_reward"])
        ep_length = rollout["episode_length"]

        episode_rewards.append(ep_rewards)
        episode_lengths.append(ep_length)
        losses.append(float(loss))
        logger.info(f"Episode {episode}/{num_episodes} - Reward: {ep_rewards:.2f}, Length: {ep_length}, Loss: {loss:.4f}")
        metdf.add_episode(seed = seed, episode = episode, reward = ep_rewards, episode_length = ep_length, loss = float(loss), algorithm = "REINFORCE", lr = learning_rate, gamma = gamma)
